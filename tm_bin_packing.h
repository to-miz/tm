...
#define _TM_BIN_PACKING_H_INCLUDED_
    ...
    typedef struct { int left, top, width, height; } tmbp_rect_;
    typedef tmbp_rect_ tmbp_rect;
    typedef size_t tmbp_size_t;
    typedef struct { int width, height; } tmbp_dim_;
...
// creates a BinPack by allocating memory using allocateFunc and storing allocatorState and reallocateFunc into the resulting
// BinPack for dynamic resizing
// params
//  allocatorState: if you are using a stateful allocator, pass in your allocator state. The state will be passed back into
//                  the allocation functions supplied
//  allocateFunc:   function pointer to an allocation function. See BinPackAllocate for the function signature
//  reallocateFunc: function pointer to a reallocation function. This is used when dynamically resizing the internal arrays.
//                  See BinPackReallocate for the function signature
...
TMBP_DEF BinPack binPackCreateEx(tmbp_int width, tmbp_int height, void* allocatorState, BinPackAllocate* allocateFunc,
                                 BinPackReallocate* reallocateFunc, BinPackFree* freeFunc, tmbp_size_t initialFreeRects,
                                 tmbp_size_t initialUsedRects);
...
TMBP_DEF BinPack binPackCreateStatic(tmbp_int width, tmbp_int height, tmbp_rect* freeRects, tmbp_size_t freeRectsCount,
                                     tmbp_rect* usedRects, tmbp_size_t usedRectsCount);
...
TMBP_DEF BinPackResult maxRectsInsertChoice(BinPack* pack, tmbp_int width, tmbp_int height,
                                            MaxRectsFreeRectChoiceHeuristic freeChoice, tmbp_bool canFlip);

// inserts multiple rects into pack at once while consuming rect dimensions from dims
// note that the order of elements in dims will be altered
// this is different than inserting bins into pack in a loop, the bins will be inserted in the order of best to lowest
// heuristic score see NOTES for more information
TMBP_DEF tmbp_size_t maxRectsInsertBatch(BinPack* pack, BinPackBatchDim* dims, BinPackBatchResult* results,
                                         tmbp_size_t count, MaxRectsFreeRectChoiceHeuristic freeChoice, tmbp_bool canFlip);
...
inline tmbp_bool binPackHasSpace(BinPack* pack) { return pack->usedRects.size < pack->usedRects.capacity; }
inline tmbp_bool guillotineHeuristicIsValidResult(tmbp_int score) { return score != TMBP_INVALID_SCORE; }
...
static tmbp_rect* tmbp_push(tmbp_rect_array* array, BinPackAllocator* allocator) {
    ...
    if(array->size >= array->capacity) {
        if(allocator->reallocateFunc) {
            tmbp_size_t newSize = (tmbp_size_t)((array->capacity + 1) * 1.5);
...
static void tmbp_unstable_erase(tmbp_rect_array* array, tmbp_size_t index) {
    TMBP_ASSERT(array);
    TMBP_ASSERT(array->data);
    TMBP_ASSERT(array->size > 0);
    TMBP_ASSERT(array->size <= array->capacity);
...
TMBP_DEF BinPack binPackCreateStatic(tmbp_int width, tmbp_int height, tmbp_rect* freeRects, tmbp_size_t freeRectsCount,
    ...
    result.allocator.state      = TMBP_NULL;
    result.allocator.reallocateFunc = TMBP_NULL;
    binPackInit(&result, width, height);
    return result;
}
TMBP_DEF void binPackInit(BinPack* pack, tmbp_int width, tmbp_int height) {
...
static tmbp_rect binPackRectFromPosition(BinPack* pack, tmbp_int width, tmbp_int height, tmbp_size_t index,
                                         tmbp_bool flipped) {
    tmbp_rect rect     = {0};
...
TMBP_DEF MaxRectsHeuristicResult maxRectsBestShortSideFit(BinPack* pack, tmbp_int width, tmbp_int height) {
    ...
    for(tmbp_size_t i = 0; i < freeRectsCount; ++i) {
        ...
        if(width <= current->width && height <= current->height) {
            ...
            if(shortSideFit < result.scores[0] || (shortSideFit == result.scores[0] && longSideFit < result.scores[1])) {
                result.freeRectIndex = i;
...
TMBP_DEF MaxRectsHeuristicResult maxRectsBestLongSideFit(BinPack* pack, tmbp_int width, tmbp_int height) {
    ...
    for(tmbp_size_t i = 0; i < freeRectsCount; ++i) {
        ...
        if(width <= current->width && height <= current->height) {
            ...
            if(longSideFit < result.scores[1] || (longSideFit == result.scores[1] && shortSideFit < result.scores[0])) {
                result.freeRectIndex = i;
                result.scores[0]     = shortSideFit;
                result.scores[1]     = longSideFit;
                result.flipped       = 0;
...
TMBP_DEF MaxRectsHeuristicResult maxRectsContactPointRule(BinPack* pack, tmbp_int width, tmbp_int height) {
    ...
    for(tmbp_size_t i = 0; i < freeRectsCount; ++i) {
        ...
        if(width <= current->width && height <= current->height) {
            tmbp_int score =
                -maxRectsContactPointScore(pack, current->left, current->top, current->left + width, current->top + height);
        ...
        if(height <= current->width && width <= current->height) {
            tmbp_int score =
                -maxRectsContactPointScore(pack, current->left, current->top, current->left + height, current->top + width);
            if(score < result.scores[0]) {
                result.freeRectIndex = i;
                result.scores[0]     = score;
                result.flipped       = 1;
...
static tmbp_bool maxRectsSplitFreeNode(BinPack* pack, tmbp_rect freeNode, const tmbp_rect* usedNode) {
