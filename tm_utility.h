/*
tm_utility v1.1.2c - public domain - https://github.com/to-miz/tm
...
*/
	...
	#endif
#endif // defined(TM_UTILITY_IMPLEMENTATION)
...
#endif // !defined(TM_USE_OWN_BEGIN_END)
...
// used to see if we are 64bit or 32bit
	...
	#endif
#endif // !defined(TMUT_POINTER_SIZE)
...
template <class T>
inline TMUT_CONSTEXPR const T& min(const T& a, const T& b) {
	return (a < b) ? (a) : (b);
}
template <class T>
inline TMUT_CONSTEXPR const T& max(const T& a, const T& b) {
	return (a < b) ? (b) : (a);
}

template <class T>
inline TMUT_CONSTEXPR const T& min(const T& a, const T& b, const T& c) {
...
inline TMUT_CONSTEXPR const T& min(const T& a, const T& b, Compare comp) {
	return comp(a, b) ? (a) : (b);
}
template <class T, class Compare>
inline TMUT_CONSTEXPR const T& max(const T& a, const T& b, Compare comp) {
	return comp(a, b) ? (b) : (a);
}
...
#endif // (defined(TMUT_SAFE_COUNTOF) || defined(_DEBUG)) && !defined(TMUT_SAFE_COUNTOF_ALT)
...
#define get_variant_or_default(tagged, name, def) (((tagged).type == typeof(tagged)::type_##name) ? ((tagged).name) : (def))
#endif
...
TMUT_CONSTEXPR inline unsigned int toupper(unsigned int cp) { return (cp >= 'a' && cp <= 'z') ? ('A' + (cp - 'a')) : cp; }
TMUT_CONSTEXPR inline unsigned int tolower(unsigned int cp) { return (cp >= 'A' && cp <= 'Z') ? ('a' + (cp - 'A')) : cp; }
...
inline ReturnType indexof(const Container& container, const ValueType& value) {
	...
	// TODO: to be perfectly correct, we'd have to use std::addressof and std::less here instead of &, < and >=
	// but then again, don't overload addressof & operator and on most platforms/compilers std::less is
	// implemented as plain operator <, so this should be fine as an assert
	...
	// of pointers, defined behavior if above assert holds
